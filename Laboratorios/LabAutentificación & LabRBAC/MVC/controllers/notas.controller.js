...
exports.get_lista = async (req, res) => {
    try {
        ...
        res.render('notas/lista', { notas: notas, permisos: req.session.permisos || [], username: req.session.username || null });
    } catch (e) {
        console.error(e);
...
exports.get_form_crear = (req, res) => {
    res.render('notas/crear', { permisos: req.session.permisos || []});
};

exports.post_crear = async (req, res) => {
    try{
        const nota = new model.Nota(req.body.titulo, req.body.contenido, req.session.username);
        await nota.crear();
        res.redirect('/notas');
    } catch (e) {
        console.error(e);
...
exports.get_form_editar = async (req, res) => {
    try {
        ...
        res.render('notas/editar', { nota: nota, permisos: req.session.permisos || [] });
    } catch (e) {
        console.error(e);
...
exports.post_editar = async (req, res) => {
    try {
        const nota = await model.Nota.actualizar( req.params.id, req.body.titulo, req.body.contenido);
        res.redirect('/notas');
    } catch (e) {
        console.error(e);
